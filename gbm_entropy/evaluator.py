...
class WDREstimator:
    ...
    def estimate(
        ...
        for stay_id in df['stay_id'].unique():
            ...
            target_actions = agent.select_actions(traj_states, temperature=temperature)
...
class ClinicalValidator:
    ...
    def __init__(self, agent, state_cols: list, action_simplifier, entropy_config):
        self.agent = agent
    ...
    def _test_bp(self, baseline: np.ndarray) -> Dict:
        """Test BP sensitivity with entropy sampling."""
        ...
        for bp in bp_range:
            state = baseline.copy()
            state[0, idx] = bp
            action = self.agent.select_actions(state, temperature=self.entropy_config.temperature)[0]
    ...
    def _test_lactate(self, baseline: np.ndarray) -> Dict:
        """Test lactate sensitivity with entropy sampling."""
    ...
    def _test_diversity(self) -> Dict:
        """Test action diversity with entropy sampling."""
        random_states = np.random.randn(2000, len(self.state_cols))
        actions = self.agent.select_actions(random_states, temperature=self.entropy_config.temperature)

        n_unique = len(np.unique(actions))
        counts = np.bincount(actions, minlength=self.agent.n_actions)
        probs = counts / counts.sum()
        entropy = -np.sum(probs * np.log(probs + 1e-10))

        passed = n_unique >= 6 and entropy > 1.2
