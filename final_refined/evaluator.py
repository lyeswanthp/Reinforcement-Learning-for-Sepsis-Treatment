...
class WDREstimator:
    ...
    def estimate(
        ...
        for stay_id in df['stay_id'].unique():
            ...
            target_actions = agent.select_actions(traj_states, use_noise=use_noise)
...
class ClinicalValidator:
    ...
    def __init__(self, agent, state_cols: list, action_simplifier, use_noise: bool = False):
        self.agent = agent
    ...
    def _test_bp(self, baseline: np.ndarray) -> Dict:
        ...
        for bp in bp_range:
            state = baseline.copy()
            state[0, idx] = bp
            action = self.agent.select_actions(state, use_noise=self.use_noise)[0]
    ...
    def _test_diversity(self) -> Dict:
        ...
        actions = self.agent.select_actions(random_states, use_noise=self.use_noise)
