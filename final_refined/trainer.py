...
class IQLTrainer:
    ...
    def train(self, train_data: dict, val_data: dict) -> dict:
        ...
        logger.info("=" * 60)
        logger.info(f"LR: {self.config.iql.learning_rate}")
        logger.info(f"Expectile: {self.config.iql.expectile}")
        logger.info(f"Entropy: {self.config.iql.entropy_weight}")
        logger.info(f"BP: {self.config.iql.bp_weight}")
        logger.info(f"Lactate: {self.config.iql.lactate_weight}")
        ...
        dataloader = DataLoader(
            ...
            shuffle=True,
            num_workers=0,
            pin_memory=(self.agent.device.type == 'cuda')
        )
