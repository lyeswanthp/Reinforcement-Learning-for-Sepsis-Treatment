"""Implicit Q-Learning agent with multi-task learning."""
...
class MultiTaskNetwork(nn.Module):
    """Multi-task network: Q-value, V-value, BP prediction, Lactate prediction."""
...
class IQLAgent:
    ...
    def __init__(self, n_states: int, n_actions: int, config):
        ...
        logger.info(f"IQL agent initialized:")
    ...
    def update(
        ...
        action_probs = F.softmax(q_pred.unsqueeze(0) / self.config.iql.temperature, dim=-1)
